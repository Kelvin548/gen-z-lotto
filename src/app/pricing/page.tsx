    ...
    <div className="min-h-screen bg-[#0b0b0c] text-white relative overflow-hidden flex flex-col selection:bg-yellow-500 selection:text-black">
      ...
      <header className="relative z-20 flex items-center justify-between px-8 py-6 max-w-7xl mx-auto w-full border-b border-zinc-900">
        ...
        <nav className="hidden md:flex items-center gap-8 text-xs font-semibold text-zinc-300">
          ...
          <Link href="/pricing" className="text-yellow-400 font-bold transition">Pricing & Odds</Link>
      ...
      {/* Content */}
      <main className="relative z-10 flex-1 max-w-5xl mx-auto px-6 py-12 space-y-8 w-full">
        <div>
          ...
          <h1 className="text-4xl font-black text-white mt-3">Pricing & Odds</h1>
          <p className="text-zinc-400 text-sm mt-2">Minimum stake starts at GH¢ 1.00 (100 pesewas). Payout multipliers are transparently calculated per game type.</p>
        </div>

        {/* Pricing Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Direct 1 Card */}
          <div className="bg-zinc-950/80 backdrop-blur-xl p-8 rounded-3xl border border-yellow-500/20 shadow-2xl space-y-4 hover:border-yellow-500/40 transition">
            <div>
              <h2 className="text-xl font-bold text-yellow-400">Direct 1</h2>
              <p className="text-zinc-300 text-sm mt-1">Match 1 exact winning number out of 5 drawn.</p>
            </div>
            <div className="pt-2">
              <span className="text-4xl font-black text-white">240x</span>
              <span className="text-zinc-400 text-sm ml-2">Multiplier</span>
            </div>
          </div>
          ...
          <div className="bg-zinc-950/80 backdrop-blur-xl p-8 rounded-3xl border border-yellow-500/20 shadow-2xl space-y-4 hover:border-yellow-500/40 transition">
            <div>
              <h2 className="text-xl font-bold text-yellow-400">Direct 2</h2>
              <p className="text-zinc-300 text-sm mt-1">Match 2 exact winning numbers drawn.</p>
            </div>
            <div className="pt-2">
              <span className="text-4xl font-black text-white">2,400x</span>
              <span className="text-zinc-400 text-sm ml-2">Multiplier</span>
            </div>
          </div>
        </div>
